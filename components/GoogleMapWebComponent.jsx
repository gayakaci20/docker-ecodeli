    ...
    for (const pkg of packages) {
      try {
        ...
        const pickupResult = await new Promise((resolve, reject) => {
          geocoder.geocode({ address: pkg.pickupAddress }, (results, status) => {
            if (status === 'OK' && results[0]) {
              resolve(results[0].geometry.location);
            } else {
              reject(`Geocode error for ${pkg.pickupAddress}: ${status}`);
            }
          });
        });
        ...
        const sizeLabel = pkg.sizeLabel || 'M'; // Taille par défaut si pas définie

        // Créer le marqueur personnalisé avec bulle de prix
        const pickupMarker = new google.maps.Marker({
          position: pickupResult,
          map: mapInstance,
          title: `${pkg.title} - ${price}€ ${sizeLabel}`,
          icon: {
        ...
        const infoWindow = new google.maps.InfoWindow({
          content: `
            <div style="max-width: 220px; font-family: 'Poppins', sans-serif;">
              <h3 style="margin: 0 0 8px 0; font-size: 14px; font-weight: 600; font-family: 'Poppins', sans-serif;">${pkg.title}</h3>
              ...
              <p style="margin: 4px 0; font-size: 12px; font-family: 'Poppins', sans-serif;"><strong>Départ:</strong> ${pkg.pickupAddress}</p>
              <p style="margin: 4px 0; font-size: 12px; font-family: 'Poppins', sans-serif;"><strong>Arrivée:</strong> ${pkg.deliveryAddress}</p>
